...
//    }
    ...
    //for member_user register


    public function member_register($email, $password, $username = false, $additional_data = false, $group_name = false)
    {
        ...
        $id = $this->db->insert_id();
        $data = array($this->meta_join => $id);
        $data1 = array_merge($additional_data[0], $data);

        $date = Date::convertDate($data1['birth_date']);
        $additional = array(
            ...
            'height' => $data1['height'],
            'weight' => $data1['weight'],
            'shoe_size' => $data1['shoe_size'],
        );
        $this->db->insert($this->tables['meta'], $additional);
        $_SESSION['user_id'] = $additional['user_id'];
        return $this->db->affected_rows() > 0 ? $id : false;
    }

    public function getMemberInfo($identity)
    {
        $sql = sprintf("select  meta.first_name,meta.last_name,meta.title,meta.skill ,meta.birth_date,meta.notes,meta.height,meta.weight,meta.shoe_size from meta INNER JOIN users ON meta.user_id=users.id where users.email= '$identity'");
